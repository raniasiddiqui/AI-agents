...
class PlannerAgentOSS(GroqOSSAgent):
    def __init__(self):
        ...
        - Title example ("Test Login with Valid Credentials")
        - Test Case Details  (Functionality being tested, including all possible variations. High-level overview)
...
def parse_and_export_testcases(test_cases_str: str):
    ...
    for num, name, content in test_matches:
        data = {
            'Test_Number': num.strip(),
            'Name': name.strip(),
            'Test Case Details': '',
            'Preconditions': '',
            'Steps': '',
        ...
        field_patterns = {
            'Test Case Details': r'-\s*(?:\*\*)?Description(?:\*\*)?\s*:\s*(.+?)(?=\n\s*-\s*|\Z)',
            'Preconditions': r'-\s*(?:\*\*)?Preconditions(?:\*\*)?\s*:\s*(.+?)(?=\n\s*-\s*|\Z)',
            # 'Test_Case_Details': r'-\s*(?:\*\*)?Test\s*Case\s*Details(?:\*\*)?\s*:\s*(.+?)(?=\n\s*-\s*|\Z)',
            'Steps': r'-\s*(?:\*\*)?Steps(?:\*\*)?\s*:\s*((?:.|\n)*?)(?=\n\s*-\s*|\Z)',
            'Expected_Result': r'-\s*(?:\*\*)?Expected\s*Result(?:\*\*)?\s*:\s*(.+?)(?=\n\s*-\s*|\Z)',
        }
        ...
        for key, pattern in field_patterns.items():
            ...
            if match:
                ...
                # Clean markdown and extra spaces
                value = re.sub(r'\*\*|_+|`+', '', value)
                if key == "Steps":
                    # Preserve step formatting with line breaks
                    value = re.sub(r'\n\s*(\d+\.)\s*', r'\n\1 ', value)
                else:
                    # For other fields, collapse newlines into spaces
                    value = re.sub(r'\s*\n\s*', ' ', value)
                data[key] = value.strip()

        all_data.append(data)

    # --- STEP 3: Export to Excel (No changes needed here)
    if all_data:
        df = pd.DataFrame(all_data)
        output_path = "cleaned_generatedtest_cases.xlsx"
        df.to_excel(output_path, index=False)

        # Format Excel cells (wrap text and adjust width)
        ...
        for col in ws.columns:
            ...
            for cell in col:
                ...
                if cell.value:
                    # Calculate max length for column width adjustment
                    lines = str(cell.value).split('\n')
                    max_length = max(max_length, *[len(line) for line in lines])
            # Set a reasonable column width, with a max limit
            ws.column_dimensions[column].width = min(max_length + 5, 70)
        wb.save(output_path)
        print(f"✅ Test cases exported successfully to {output_path}")
    else:
        print("⚠️ No test cases parsed — please check input format.")
...
async def main():
    ...
    parse_and_export_testcases(test_cases)
