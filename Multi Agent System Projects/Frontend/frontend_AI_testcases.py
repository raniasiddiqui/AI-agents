...
class PlannerAgentOSS(GroqOSSAgent):
    def __init__(self):
        ...
        1.  Use the exact sub-headings: `Test Case ID`, `Title`, `Testing Type`, `Test Case`, `Step-by-step actions`, and `Expected Result`.
        2.  Number the Test Case ID sequentially starting from 1 (e.g., TC-1, TC-2, etc.).
        3. The fields `Test Case ID`, `Title`, `Testing Type`, `Test Case`, and `Expected Result` MUST be single lines using the bullet (`*`) prefix.
...
def parse_and_export_testcases(test_cases_str: str):
    ...
    for num, title, content in test_matches:
        data = {
            'Test Case ID': '',
            'Title': title.strip(),
            'Testing_Type': '',
...
async def run_feedback_generation(feedback_prompt, status_placeholder):
    ...
    planner_input = (
        ...
        f"- Expected Result\n\n"
        f"**GUIDELINES:**\n"
        f"- Use the exact subheadings: `Test Case ID`, `Title`, `Testing Type`, `Test Case`, `Step-by-step actions`, and `Expected Result`. "
        f"- Number the Test Case ID sequentially starting from the last generated Test Case ID.\n"
        f"- The fields `Test Case ID`, `Title`, `Testing Type`, `Test Case`, and `Expected Result` MUST be single lines using the bullet (`*`) prefix.\n"
...
3. Use `* ` prefix for every field (Title, Testing Type, Test Case, Step-by-step actions, Expected Result).
